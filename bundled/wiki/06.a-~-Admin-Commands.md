...
_Alias:_ `mcdev rb`, `mcdev refreshBUs`
...
_Command:_ `mcdev resolveId <business unit> <metadata type> <ID> [--json]`

_Alias:_ `mcdev rid`

Looks up metadata by **numeric ID** on the server (for example an asset ID) and prints the key, name, and sharing details when applicable. This is particularly helpful when you work with `ContentBlockById` and need the matching key or path.

Add `--json` for machine-readable output instead of the default table-style log.

_Example:_

```bash
mcdev rid MyProject/DEV asset 4961
mcdev rid MyProject/DEV asset 4961 --json
```
...
_Command:_ `mcdev describeSoap <TYPE> [--bu] [--json]`
...
Optional **`--bu`** selects which Business Unit context to use (defaults follow the tool’s parent-BU behavior). **`--json`** prints the same field list as JSON instead of a text table.

_Example:_

```bash
mcdev soap DataFolder
mcdev soap DataFolder --bu MyProject/_ParentBU_
mcdev soap DataFolder --json
```
...
_Command:_ `mcdev getFilesToCommit <business unit> <metadata type> <metadata key>`

_Alias:_ `mcdev fc`

Prints the list of **relative file paths** you should include in a Git commit for the given metadata component—useful when only part of the tree changed and you want a precise commit scope.

_Example:_

```bash
mcdev fc MyProject/DEV dataExtension myKey
```

> Note: The CLI may gain a `--metadata` style selector in a future release; today the primary form uses the three positionals above.