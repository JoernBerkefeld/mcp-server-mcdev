...
**retrieve with --no-purge:**

Because `--purge` is a boolean flag, you can pass `--no-purge` to decline deleting folders without an interactive prompt when the tool would otherwise ask.
...
_Command_: `mcdev deploy [business unit] [metadata type] [metadata key] [--metadata] [--fromRetrieve] [--refresh] [--keySuffix] [--autoMidSuffix] [--no-autoMidSuffix] [--changeKeyValue=yourNewKey] [--changeKeyField=otherFieldInJson] [--execute] [--schedule] [--fixShared] [--noUpdate] [--publish] [--validate] [--skipStatusCheck] [--matchName] [--ignoreFolder] [--skipValidation] [--format] [--no-format] [--ignoreSfFields] [--fix]`
...
**deploy with --fix option:**

When a validation rule supports automatic correction during deploy, pass `--fix` so mcdev can apply it instead of only reporting the issue.

```bash
mcdev deploy MyProject/DEV dataExtension --fix
mcdev deploy MyProject/DEV -m script:myKey --fix
```
...
_Command:_ `mcdev fixKeys <business unit> [type] [key] [--metadata] [--like] [--keySuffix] [--execute] [--schedule]`
...
Use **`--keySuffix`** (or **`-ks`**) when you need the updated key to carry a custom suffix, similar in spirit to deploy’s key-suffix options.
