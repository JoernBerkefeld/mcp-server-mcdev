...
These flags apply to **any** `mcdev` subcommand unless noted otherwise.

### --verbose

Increases detail in CLI output.

### --debug

Enables developer-oriented behavior and edge-case features where the tool supports them.

### --silent

Limits CLI output to errors only.

### --skipInteraction (aliases: `--yes`, `-y`)

Skips interactive prompts and uses defaults where the tool would otherwise ask. Often combined with command-specific `--y.*` answers (for example during `init` or `fixKeys`). See the relevant command sections for required companion flags.

### --api=[log|cli]

Optionally, you can see API requests and responses in the log file or directly in the CLI (`cli`).
