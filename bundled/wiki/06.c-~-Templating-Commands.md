...
| `--purge` | - | Deletes the relevant deploy folder before the build step. Use `--no-purge` to skip the confirmation and leave existing deploy output in place |
...
_Command:_ `mcdev build <--buFrom> <--marketFrom> <--buTo> <--marketTo> <--metadata> [--bulk] [--dependencies] [--retrieve] [--skipValidation] [--purge] [--no-purge] [--fix]`
...
**build with --purge and --no-purge:**

`--purge` removes the deploy folder content that applies to this run before `buildDefinition` runs, so you start from a clean target folder. Pass `--no-purge` to skip the confirmation and avoid deleting anything when the tool would otherwise prompt.

**build with --fix:**

When validation rules support automatic fixes during the build path, `--fix` lets mcdev apply them (same idea as on `buildDefinition`).
...
_Command:_ `mcdev buildDefinition <business unit> [type] [template name] [market] [--metadata] [--market] [--skipValidation] [--purge] [--no-purge] [--fix]`
...
| `--no-purge` | Skip the confirmation and do not delete deploy output first |
| `--skipValidation` | Skip validation rules during build |
| `--fix` | Apply automatic fixes where validation rules support it |
...
_Command:_ `mcdev buildDefinitionBulk <market list name> <type> <template name> [--metadata] [--skipValidation] [--purge] [--no-purge] [--fix]`
